...
const getDateInfo = (dt) => {
  ...
  if (isToday(d)) {
    ...
    if (h <= 0) return { label:"Due now",   color:"var(--danger)",  bg:"var(--danger-soft)" };
    if (h < 3)  return { label:`In ${h}h`,  color:"var(--warning)", bg:"var(--warning-soft)" };
    return { label:"Today", color:"var(--accent)", bg:"var(--accent-soft)" };
  }
  if (isTomorrow(d)) return { label:"Tomorrow", color:"var(--success)", bg:"var(--success-soft)" };
  if (isPast(d))     return { label:"Overdue",  color:"var(--danger)",  bg:"var(--danger-soft)" };
...
export default function ReminderCard({ reminder, onEdit }) {
  ...
  return (
    <div className="scale-in" style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderLeft:`4px solid ${reminder.completed?"var(--success)":isOverdue?"var(--danger)":"var(--accent)"}`, borderRadius:"12px", padding:"14px 16px", boxShadow:"var(--shadow)", opacity: reminder.completed?0.65:1, transition:"box-shadow 0.15s" }}
      onMouseEnter={e=>e.currentTarget.style.boxShadow="var(--shadow-lg)"}
      onMouseLeave={e=>e.currentTarget.style.boxShadow="var(--shadow)"}
    >
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"12px" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"4px", flexWrap:"wrap" }}>
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:"15px", fontWeight:"600", color:"var(--text-primary)", textDecoration:reminder.completed?"line-through":"none", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", maxWidth:"100%" }}>
              {reminder.title}
            </h3>
            <span style={{ fontSize:"11px", fontWeight:"700", color:reminder.completed?"var(--success)":info.color, background:reminder.completed?"var(--success-soft)":info.bg, padding:"2px 9px", borderRadius:"99px", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"14px", flexWrap:"wrap", marginTop:"4px" }}>
            <span style={{ display:"flex", alignItems:"center", gap:"4px", fontSize:"12px", color:"var(--text-muted)" }}>
              <RiCalendarLine size={13}/> {format(new Date(reminder.dateTime),"EEE, MMM d yyyy")}
            </span>
            <span style={{ display:"flex", alignItems:"center", gap:"4px", fontSize:"12px", color:"var(--text-muted)" }}>
              <RiTimeLine size={13}/> {format(new Date(reminder.dateTime),"h:mm a")}
            </span>
          </div>
        </div>
        ...
        <div style={{ display:"flex", gap:"6px", flexShrink:0, alignItems:"center" }}>
          {!reminder.completed && (
            <>
              <button onClick={()=>onEdit(reminder)} title="Edit" style={{ width:"34px", height:"34px", borderRadius:"8px", border:"1px solid var(--border)", background:"var(--bg-input)", color:"var(--text-secondary)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <RiEditLine size={15}/>
              </button>
              <button onClick={handleComplete} disabled={completing} title="Mark complete" style={{ width:"34px", height:"34px", borderRadius:"8px", border:"1px solid var(--success)", background:"var(--success-soft)", color:"var(--success)", cursor:completing?"not-allowed":"pointer", opacity:completing?0.6:1, display:"flex", alignItems:"center", justifyContent:"center" }}>
                <RiCheckLine size={15}/>
              </button>
            </>
          )}
          <button onClick={handleDelete} disabled={deleting} title="Delete" style={{ width:"34px", height:"34px", borderRadius:"8px", border:"1px solid var(--danger)", background:"var(--danger-soft)", color:"var(--danger)", cursor:deleting?"not-allowed":"pointer", opacity:deleting?0.6:1, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <RiDeleteBinLine size={15}/>
          </button>
        </div>
      </div>
