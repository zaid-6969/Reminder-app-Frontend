...
export const markIncomplete = createAsyncThunk(
  "reminders/markIncomplete",
  async (id, { rejectWithValue }) => {
    try {
      const res = await api.patch(`/reminders/${id}/incomplete`);
      return res.data;
    } catch (err) {
      return rejectWithValue(
        err.response?.data?.message || "Failed to update reminder",
...
const reminderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    // Fetch
    builder
      .addCase(fetchReminders.pending, (s) => {
        s.loading = true;
      })
    ...
    builder
      .addCase(createReminder.pending, (s) => {
        s.submitting = true;
      })
    ...
    builder
      .addCase(updateReminder.pending, (s) => {
        s.submitting = true;
      })
    ...
    // Mark incomplete — update in both lists
    builder
      .addCase(markIncomplete.fulfilled, (s, a) => {
        const updated = a.payload.reminder;
        ...
        // If viewing completed only, remove it from visible list
      ...
      .addCase(markIncomplete.rejected, (_, a) => toast.error(a.payload));
  },
});
